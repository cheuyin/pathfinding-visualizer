  ...
  switch (action.type) {
    ...
    case 'SET_SOURCE': {
      return state.map((row) =>
        row.map((n) => {
          if (n.x === action.coord.x && n.y === action.coord.y)
            return { ...n, type: NodeType.SOURCE };
    ...
    case 'CLEAR_VISUALIZATION': {
      return state.map((row) =>
        row.map((n) =>
          n.type === NodeType.VISITED || n.type === NodeType.PATH
            ? { ...n, type: NodeType.BLANK }
            : n,
        ),
      );
    }
